...
function CanceledConsultation({ show, onHide }) {
  ...
  const initialLoad = async () => {
    try {
      const result = await CanceledConsulationAPI();
      if (result?.data) {
    ...
    <Modal show={show} fullscreen={true} onHide={() => onHide()}>
      ...
      >
        <Modal.Header closeButton>
          <Modal.Title>Cancelled Consultations</Modal.Title>
        </Modal.Header>
        <ConsultDetails
          show={detailsModal}
          onHide={(e) => {
...
export default CanceledConsultation;
